...
DROP TABLE IF EXISTS teacher_subject_levels CASCADE;
DROP TABLE IF EXISTS student_subject_levels CASCADE;
...
-- Insert all subject-level combinations with default price
INSERT INTO subject_levels (subject_id, level_id, price_per_month)
SELECT s.id, l.id, 50.00
FROM subjects s
CROSS JOIN levels l;
